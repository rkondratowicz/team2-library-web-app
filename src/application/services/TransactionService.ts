...
export class TransactionService {
  ...
  async returnBook(
    ...
    const updatedTransaction = await transactionRepository.returnBook(
      transactionId,
      returnNotes
    );
  ...
  private async validateBorrowingRequest(
    request: CreateBorrowingRequest
  ): Promise<BorrowingValidation> {
    const errors: string[] = [];
    ...
    try {
      ...
      const activeBorrowing =
        await transactionRepository.getActiveBorrowingByCopy(
      ...
      const activeBorrows =
        await transactionRepository.countActiveBorrowsByMember(
          request.member_id
        );
      if (activeBorrows >= this.MAX_BOOKS_PER_MEMBER) {
        errors.push(
          `Member has reached maximum borrowing limit of ${this.MAX_BOOKS_PER_MEMBER} books`
        );
      }
  ...
  async canMemberBorrow(
    memberId: string
  ): Promise<{ canBorrow: boolean; reason?: string }> {
    try {
      const activeBorrows =
        await transactionRepository.countActiveBorrowsByMember(memberId);
  ...
  async getBorrowingStats() {
    try {
      const activeTransactions =
        await transactionRepository.getAllActiveTransactions();
      ...
      return {
        ...
        members_with_active_borrows: new Set(
          activeTransactions.map((t) => t.member_id)
        ).size,
