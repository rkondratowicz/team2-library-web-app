...
export class TransactionController {
    ...
    async borrowBook(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (!book_copy_id || !member_id) {
                res.status(400).json({ 
                    error: 'Missing required fields: book_copy_id and member_id are required' 
                });
                return;
            }
    ...
    async returnBook(req: Request, res: Response): Promise<void> {
        try {
            ...
            const transaction = await transactionService.returnBook(transactionId, return_notes);
    ...
    async getMemberBorrowingSummary(req: Request, res: Response): Promise<void> {
        try {
            ...
            const summary = await transactionService.getMemberBorrowingSummary(memberId);

            if (!summary) {
                res.status(404).json({ error: 'Member not found or no borrowing history' });
                return;
            }
            ...
            res.status(500).json({ error: 'Failed to fetch member borrowing summary' });
        }
    }
    ...
    async checkMemberBorrowingEligibility(req: Request, res: Response): Promise<void> {
        try {
            const memberId = req.params.memberId;
