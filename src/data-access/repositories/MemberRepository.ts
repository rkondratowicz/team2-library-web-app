...
export class MemberRepository {
  ...
  async create(memberData: CreateMemberRequest): Promise<Member> {
    const memberId = memberData.member_id || await this.generateNextMemberId();
    ...
    try {
      ...
      if (error.message.includes('UNIQUE constraint failed: members.member_id')) {
  ...
  async search(query: string, options: PaginationOptions = {}): Promise<Member[]> {
    const { limit = 50, offset = 0 } = options;
  ...
  async findByStatus(status: MemberStatus, options: PaginationOptions = {}): Promise<Member[]> {
    const { limit = 50, offset = 0 } = options;
  ...
  async findMembersByRegistrationDateRange(start: Date, end: Date): Promise<Member[]> {
  ...
  // Update member
  async update(id: number, updates: UpdateMemberRequest): Promise<Member | null> {
    const fields = [];
    const values = [];
  ...
  async delete(id: number): Promise<boolean> {
    const sql = 'UPDATE members SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?';
  ...
  async count(): Promise<number> {
    const result = await databaseConnection.get('SELECT COUNT(*) as count FROM members');
  ...
  async getMemberStatistics(): Promise<MemberStats> {
    const totalResult = await databaseConnection.get('SELECT COUNT(*) as count FROM members');
    const activeResult = await databaseConnection.getOne('SELECT COUNT(*) as count FROM members WHERE status = ?', [MemberStatus.ACTIVE]);
    const inactiveResult = await databaseConnection.getOne('SELECT COUNT(*) as count FROM members WHERE status = ?', [MemberStatus.INACTIVE]);
    const suspendedResult = await databaseConnection.getOne('SELECT COUNT(*) as count FROM members WHERE status = ?', [MemberStatus.SUSPENDED]);
    const newThisMonthResult = await databaseConnection.get("SELECT COUNT(*) as count FROM members WHERE registration_date >= date('now', 'start of month')");
    const newThisYearResult = await databaseConnection.get("SELECT COUNT(*) as count FROM members WHERE registration_date >= date('now', 'start of year')");
