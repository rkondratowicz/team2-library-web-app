...
export class TransactionRepository {
    ...
    async createBorrowingTransaction(request: CreateBorrowingRequest & { due_date: string }): Promise<BorrowingTransaction> {
    ...
    async getActiveBorrowingsByMember(memberId: string): Promise<BorrowingTransaction[]> {
        const query = `
    ...
    async getAllBorrowingsByMember(memberId: string): Promise<BorrowingTransaction[]> {
        const query = `
            SELECT * FROM borrowing_transactions 
            WHERE member_id = ?
            ORDER BY borrow_date DESC
    ...
    async getActiveBorrowingByCopy(copyId: string): Promise<BorrowingTransaction | null> {
        const query = `
            SELECT * FROM borrowing_transactions 
    ...
    async getTransactionWithDetails(id: string): Promise<BorrowingTransactionWithDetails | null> {
        const query = `
            SELECT 
                bt.*,
...
import { databaseConnection } from '../DatabaseConnection.js';
export const transactionRepository = new TransactionRepository(databaseConnection);